...
- If the user explicitly asks for Deep Research, choose the Deep Research preset from the canonical registry. Those jobs end with the conversation link instead of a saved report today (the report renders in a widget the worker cannot read); tell the user to open the link, and include "do not ask clarifying questions; proceed with stated assumptions" in the prompt.
