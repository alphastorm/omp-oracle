...
- If the user explicitly asks for Deep Research, choose the Deep Research preset from the canonical registry; it needs the existing-Chrome relay transport and runs for minutes. Include "do not ask clarifying questions; proceed with stated assumptions" in the prompt, because a clarifying reply fails the job instead of starting research.
