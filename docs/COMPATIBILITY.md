...
| Oh My Pi 18.2.6 | Observed loading and preflight | `omp plugin list` shows the package as an npm plugin; `oracle_preflight` reported ready with the relay transport reachable; `omp install --dry-run` accepts the checkout and the GitHub URL. No full job, platform matrix, or preset proof has been run on OMP yet |
