...
function testCanonicalWorkflowConfig() {
  ...
  assert.equal(pkg.scripts["smoke:platform:all"], `npm run smoke:platform:doctor && node scripts/platform-smoke.mjs run --target ${config.requiredTargets.join(",")}`, "full platform smoke should remain doctor-first and cover all required targets");
